...
#include "getcooperatestate_fuzzer.h"
...
bool GetCooperateStateFuzzTest(const uint8_t* data, size_t size)
{
    ...
    DelayedSingleton<DeviceStatusService>::GetInstance()->OnRemoteRequest(
        static_cast<uint32_t>(Msdp::IIntentionIpcCode::COMMAND_GET_COOPERATE_STATE_SYNC), datas, reply, option);
    return true;
}
} // namespace OHOS
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::GetCooperateStateFuzzTest(data, size);
    return 0;
}
} // namespace DeviceStatus
} // namespace Msdp
