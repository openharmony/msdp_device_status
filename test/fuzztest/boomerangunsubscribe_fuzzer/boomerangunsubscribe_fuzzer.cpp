...
#include "boomerangunsubscribe_fuzzer.h"
...
#undef LOG_TAG
#define LOG_TAG "BoomerangUnsubscribeFuzzTest"
...
bool BoomerangUnsubscribeFuzzTest(const uint8_t* data, size_t size)
{
    MessageParcel datas;
    ...
    DelayedSingleton<DeviceStatusService>::GetInstance()->OnRemoteRequest(
        static_cast<uint32_t>(Msdp::IIntentionIpcCode::COMMAND_UNSUBSCRIBE_CALLBACK), datas, reply, option);
    return true;
}
} // namespace OHOS
/* Fuzzer entry point */
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::BoomerangUnsubscribeFuzzTest(data, size);
    return 0;
}
} // namespace DeviceStatus
} // namespace Msdp
