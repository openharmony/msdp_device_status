...
#undef LOG_TAG
#define LOG_TAG "StopDragFuzzTest"
...
namespace OHOS {
bool StopDragFuzzTest(const uint8_t* data, size_t size)
{
    ...
    if (!datas.WriteInterfaceToken(FORMMGR_DEVICE_TOKEN) || !datas.WriteBuffer(data, size) || !datas.RewindRead(0)) {
        FI_HILOGE("Write datas failed");
        return false;
    }
    MessageParcel reply;
    MessageOption option;
    DelayedSingleton<DeviceStatusService>::GetInstance()->OnRemoteRequest(
        static_cast<uint32_t>(Msdp::IIntentionIpcCode::COMMAND_STOP_DRAG), datas, reply, option);
    return true;
}
} // namespace OHOS
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::StopDragFuzzTest(data, size);
    return 0;
}
} // namespace DeviceStatus
} // namespace Msdp
