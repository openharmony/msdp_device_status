...
bool GetDragDataFuzzTest(const uint8_t* data, size_t size)
{
    ...
    if (!datas.WriteInterfaceToken(FORMMGR_DEVICE_TOKEN) || !datas.WriteBuffer(data, size) || !datas.RewindRead(0)) {
        FI_HILOGE("Write datas content failed");
        return false;
    }
    MessageParcel reply;
    MessageOption option;
    DelayedSingleton<DeviceStatusService>::GetInstance()->OnRemoteRequest(
        static_cast<uint32_t>(Msdp::IIntentionIpcCode::COMMAND_GET_DRAG_DATA), datas, reply, option);
    return true;
}
} // namespace OHOS
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::GetDragDataFuzzTest(data, size);
    return 0;
}
} // namespace DeviceStatus
} // namespace Msdp
