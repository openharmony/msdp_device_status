...
namespace OHOS {
constexpr OHOS::HiviewDFX::HiLogLabel LABEL { LOG_CORE, Msdp::MSDP_DOMAIN_ID, "RegisterCoordinationListenerFuzzTest" };
const std::u16string FORMMGR_DEVICE_TOKEN { u"ohos.msdp.Idevicestatus" };

bool RegisterCoordinationListenerFuzzTest(const uint8_t* data, size_t size)
{
    MessageParcel datas;
    bool isCheckPermission = true;
    if (!datas.WriteInterfaceToken(FORMMGR_DEVICE_TOKEN) || !datas.WriteBool(isCheckPermission) ||
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::RegisterCoordinationListenerFuzzTest(data, size);
    return 0;
}
