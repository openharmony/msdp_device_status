...
#undef LOG_TAG
#define LOG_TAG "GetDragTargetPidFuzzTest"
...
bool GetDragTargetPidFuzzTest(const uint8_t* data, size_t size)
{
    ...
    if (!datas.WriteInterfaceToken(FORMMGR_DEVICE_TOKEN) || !datas.WriteBuffer(data, size) || !datas.RewindRead(0)) {
        FI_HILOGE("Failed to write datas content");
        return false;
    }
    MessageParcel reply;
    MessageOption option;
    DelayedSingleton<DeviceStatusService>::GetInstance()->OnRemoteRequest(
        static_cast<uint32_t>(Msdp::IIntentionIpcCode::COMMAND_GET_DRAG_TARGET_PID), datas, reply, option);
    return true;
}
} // namespace OHOS
