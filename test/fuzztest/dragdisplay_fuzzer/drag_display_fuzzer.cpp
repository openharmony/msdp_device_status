...
#undef LOG_TAG
#define LOG_TAG "DragDisplayFuzzTest"
...
bool UpdatePreviewStyleStubFuzzTest(FuzzedDataProvider &provider)
{
...
bool UpdatePreviewStyleWithAnimationStubFuzzTest(FuzzedDataProvider &provider)
{
    auto buffer = provider.ConsumeRemainingBytes<uint8_t>();
    MessageParcel datas;
    if (!datas.WriteInterfaceToken(FORMMGR_DEVICE_TOKEN) ||
...
bool AddPrivilegeStubFuzzTest(FuzzedDataProvider &provider)
{
    auto buffer = provider.ConsumeRemainingBytes<uint8_t>();
    MessageParcel datas;
    if (!datas.WriteInterfaceToken(FORMMGR_DEVICE_TOKEN) ||
...
bool EraseMouseIconStubFuzzTest(FuzzedDataProvider &provider)
{
    auto buffer = provider.ConsumeRemainingBytes<uint8_t>();
    MessageParcel datas;
    if (!datas.WriteInterfaceToken(FORMMGR_DEVICE_TOKEN) ||
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::UpdatePreviewStyleStubFuzzTest(provider);
    OHOS::UpdatePreviewStyleWithAnimationStubFuzzTest(provider);
    OHOS::AddPrivilegeStubFuzzTest(provider);
    OHOS::EraseMouseIconStubFuzzTest(provider);
    return 0;
}
} // namespace DeviceStatus
} // namespace Msdp
