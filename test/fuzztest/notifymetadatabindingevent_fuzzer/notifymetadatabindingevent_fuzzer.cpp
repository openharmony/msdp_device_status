...
#include "notifymetadatabindingevent_fuzzer.h"
...
#undef LOG_TAG
#define LOG_TAG "NotifyMetaDataBindingEventFuzzTest"
...
bool NotifyMetaDataBindingEventFuzzTest(const uint8_t* data, size_t size)
{
    MessageParcel datas;
    ...
    DelayedSingleton<DeviceStatusService>::GetInstance()->OnRemoteRequest(
        static_cast<uint32_t>(Msdp::IIntentionIpcCode::COMMAND_NOTIFY_METADATA_BINDING_EVENT), datas, reply, option);
    return true;
}
} // namespace OHOS
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::NotifyMetaDataBindingEventFuzzTest(data, size);
    return 0;
}
} // namespace DeviceStatus
} // namespace Msdp
