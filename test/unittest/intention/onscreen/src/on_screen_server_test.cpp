 ...
 * @tc.type: FUNC
 */
HWTEST_F(OnScreenServerTest, RegisterAwarenessCallback, TestSize.Level0)
{
    CALL_TEST_DEBUG;
    uint64_t g_tokenId = NativeTokenGet();
    EXPECT_EQ(g_tokenId, IPCSkeleton::GetCallingTokenID());
    ...
    int32_t ret = onScreen_.RegisterAwarenessCallback(context, cap, callback, option);
    EXPECT_NE(ret, RET_OK);
    OHOS::Security::AccessToken::AccessTokenKit::DeleteToken(g_tokenId);
}
 ...
 * @tc.type: FUNC
 */
HWTEST_F(OnScreenServerTest, UnregisterAwarenessCallback, TestSize.Level0)
{
    CALL_TEST_DEBUG;
    uint64_t g_tokenId = NativeTokenGet();
    EXPECT_EQ(g_tokenId, IPCSkeleton::GetCallingTokenID());
    ...
    AwarenessCap cap;
    sptr<IRemoteOnScreenCallback> callback = new (std::nothrow) OnScreenServerTestCallback();
    int32_t ret = onScreen_.UnregisterAwarenessCallback(context, cap, callback);
    EXPECT_NE(ret, RET_OK);
    OHOS::Security::AccessToken::AccessTokenKit::DeleteToken(g_tokenId);
}
 ...
 * @tc.type: FUNC
 */
HWTEST_F(OnScreenServerTest, Trigger, TestSize.Level0)
{
    CALL_TEST_DEBUG;
    uint64_t g_tokenId = NativeTokenGet();
    EXPECT_EQ(g_tokenId, IPCSkeleton::GetCallingTokenID());
    ...
    int32_t ret = onScreen_.Trigger(context, cap, option, info);
    EXPECT_NE(ret, RET_OK);
