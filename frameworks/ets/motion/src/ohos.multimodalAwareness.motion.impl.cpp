...
void onOperatingHandChanged(taihe::callback_view<void(OperatingHandStatus_t)> f, uintptr_t opq)
{
#ifdef MOTION_ENABLE
...
void offOperatingHandChanged(::taihe::optional_view<uintptr_t> opq)
{
#ifdef MOTION_ENABLE
    if (!opq.has_value()) {
...
TH_EXPORT_CPP_API_onOperatingHandChanged(onOperatingHandChanged);
TH_EXPORT_CPP_API_offOperatingHandChanged(offOperatingHandChanged);
// NOLINTEND
