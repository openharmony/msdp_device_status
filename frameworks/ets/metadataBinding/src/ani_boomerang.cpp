...
bool AniBoomerangEvent::IsNoExistCallback(std::list<std::shared_ptr<AniBoomerangEventListener>>,
...
bool AniBoomerangEvent::RemoveAllCallback(int32_t eventType)
{
    ...
    if (iter == events_.end()) {
        FI_HILOGE("%{public}s evenType %{public}d not found", LOG_TAG, eventType);
        return false;
    }
