    ...
    std::shared_ptr<OHOS::Media::PixelMap> &resultPixelMap)
{
    ...
    if (((error = dlerror()) != nullptr) || (handle == nullptr)) {
        FI_HILOGE("%{public}s Boomerang Algo Encode Load failed, error: %{public}s", LOG_TAG, error);
        return false;
    }
...
bool AniBoomerang::DecodeImageFunc(std::shared_ptr<OHOS::Media::PixelMap> &pixelMap, std::string &content)
{
    ...
    char *error = nullptr;
    if (((error = dlerror()) != nullptr) || (handle == nullptr)) {
        FI_HILOGE("%{public}s Boomerang Algo Decode Load failed, error: %{public}s", LOG_TAG, error);
        return false;
    }
