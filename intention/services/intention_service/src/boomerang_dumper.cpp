...
#undef LOG_TAG
#define LOG_TAG "BoomerangDumper"

namespace OHOS {
namespace Msdp {
namespace DeviceStatus {
void BoomerangDumper::Dump(int32_t fd, const std::vector<std::string> &args)
{
...
void BoomerangDumper::DumpOnce(int32_t fd, int32_t option)
{
    switch (option) {
        case 's': {
...
void BoomerangDumper::DumpHelpInfo(int32_t fd) const
{
    dprintf(fd, "Usage:\n");
...
void BoomerangDumper::DumpDeviceStatusSubscriber(int32_t fd) const
{
    CHKPV(env_);
    FI_HILOGI("Dump subscribers of device status");
    int32_t ret = env_->GetDelegateTasks().PostSyncTask([this, fd] {
        boomerang_.DumpDeviceStatusSubscriber(fd);
        return RET_OK;
    });
    if (ret != RET_OK) {
...
void BoomerangDumper::DumpDeviceStatusChanges(int32_t fd) const
{
    CHKPV(env_);
    FI_HILOGI("Dump changes of device status");
    int32_t ret = env_->GetDelegateTasks().PostSyncTask([this, fd] {
        boomerang_.DumpDeviceStatusChanges(fd);
        return RET_OK;
    });
    if (ret != RET_OK) {
        FI_HILOGE("IDelegateTasks::PostSyncTask fail, error:%{public}d", ret);
...
void BoomerangDumper::DumpCurrentDeviceStatus(int32_t fd)
{
    CHKPV(env_);
...
void BoomerangDumper::DumpDrag(int32_t fd) const
{
    CHKPV(env_);
...
void BoomerangDumper::DumpCheckDefine(int32_t fd) const
{
...
template<class ...Ts>
void BoomerangDumper::CheckDefineOutput(int32_t fd, const char* fmt, Ts... args) const
{
