...
void CooperateClient::StartTrace(int32_t userData)
{
    ...
    FI_HILOGI("[WLD_PERF] Start tracing \'%{public}d\'", userData);
}
...
void CooperateClient::FinishTrance(int32_t userData)
{
    ...
    if (auto iter = performaceInfo_.traces_.find(userData); iter != performaceInfo_.traces_.end()){
        ...
        FI_HILOGI("[WLD_PERF] Finish tracing \'%{public}d\', elapsed: %{public}lld ms", userData, curDuration);
        performaceInfo_.traces_.erase(iter);
        performaceInfo_.successNum += 1;
        performaceInfo_.failNum = performaceInfo_.traces_.size();
        performaceInfo_.successRate = (performaceInfo_.successNum * 100) / performaceInfo_.activateNum;
        ...
        FI_HILOGW("[WLD_PERF] Finish tracing with something wrong");
    }
}
...
void CooperateClient::DumpPerformaceInfo()
{
    ...
    FI_HILOGI("[WLD_PERF] performaceInfo:"
    ...
    FI_HILOGI("[WLD_PERF] Duration: %{public}s",durationStr.c_str());
}
#endif // ENABLE_PERFORMANCE_CHECK
} // namespace DeviceStatus
} // namespace Msdp
