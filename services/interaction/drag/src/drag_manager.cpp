...
int32_t DragManager::AddDragEventMonitor(int32_t sourceType)
{
    CALL_DEBUG_ENTER;
    ...
    monitorId_ = MMI::InputManager::GetInstance()->AddMonitor(consumer);
    if (monitorId_ < 0) {
...
int32_t DragManager::OnStartDrag()
{
    ...
    ret = AddDragEventMonitor(dragData.sourceType);
    if (ret != RET_OK) {
