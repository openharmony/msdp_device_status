...
void DragDrawing::DoFollowHandAnimation(const float &displayX, const float &displayY)
{
    ...
    CalculateLightIntensity(degreeX, degreeY, lightIntensity);
    auto parentNode = g_drawingInfo.parentNode;
...
void DragDrawing::StopDestopAnimation()
{
    ...
    if (dragWindowVisible_.load()) {
        ...
        g_drawingInfo.parentNode->SetRotation(0.0f, 0.0f, DragWindowRotateInfo_.rotation);
...
void DragDrawing::GetDragDrawingInfo(DragInternalInfo &dragInternalInfo)
{
    ...
    sptr<Rosen::DisplayInfo> display = Rosen::DisplayManager::GetInstance().GetVisibleAreaDisplayInfoById(
        g_drawingInfo.displayId);
    if (display == nullptr) {
        FI_HILOGD("Get display info failed, display:%{public}d", g_drawingInfo.displayId);
...
int32_t DragDrawing::MoveToEndAnimation()
{
    ...
    parentNode->SetRotation(0.0f, 0.0f, DragWindowRotateInfo_.rotation);
...
int32_t DragDrawing::RunDestopAnimation(int32_t pid, std::string dragAnimationInfo)
{
    ...
    Rosen::RSAnimationTimingProtocol protocol;
    protocol.SetDuration(ANIMATION_DURATION);
    parentNode->SetRotation(rotationDegreeX_, rotationDegreeY_, DragWindowRotateInfo_.rotation);
