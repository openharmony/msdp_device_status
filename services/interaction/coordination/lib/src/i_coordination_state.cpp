...
int32_t ICoordinationState::PrepareAndStart(const std::string &srcNetworkId, int32_t startDeviceId)
{
    ...
    if (NeedPrepare(srcNetworkId, sinkNetworkId)) {
        ...
        if (ret != RET_OK) {
            ...
            InputDevCooSM->OnStartFinish(false, sinkNetworkId, startDeviceId);
    ...
    bool isSuccess, const std::string &srcNetworkId, int32_t startDeviceId)
{
    ...
    if (!isSuccess) {
        InputDevCooSM->UpdatePreparedDevices("", "");
        InputDevCooSM->OnStartFinish(false, srcNetworkId, startDeviceId);
    } else {
...
int32_t ICoordinationState::StartRemoteInput(int32_t startDeviceId)
{
    ...
    if (dhids.empty()) {
        InputDevCooSM->OnStartFinish(false, networkIds.first, startDeviceId);
    ...
    bool isSuccess, const std::string &srcNetworkId, int32_t startDeviceId)
{
    ...
    std::function<void()> handleStartFinishFunc =
        std::bind(&CoordinationSM::OnStartFinish, InputDevCooSM, isSuccess, srcNetworkId, startDeviceId);
    CHKPV(eventHandler_);
