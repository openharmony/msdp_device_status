...
int32_t DragDataPacker::Marshalling(const DragData &dragData, Parcel &data)
{
    ...
    if (MarshallingSummary(dragData.summarys, data) != RET_OK) {
...
int32_t DragDataPacker::UnMarshalling(Parcel &data, DragData &dragData)
{
    ...
    if (UnmarshallingSummary(data, dragData.summarys) != RET_OK) {
...
int32_t DragDataPacker::MarshallingShadowInfos(const std::vector<ShadowInfo> &shadowInfos, Parcel &data)
{
    CALL_DEBUG_ENTER;
    ...
    if (shadowNum > SHADOW_NUM_LIMIT) {
        FI_HILOGW("Only %{public}d shadowInfos allowed, %{public}d exceeding the limit", SHADOW_NUM_LIMIT, shadowNum);
...
int32_t DragDataPacker::UnMarshallingShadowInfos(Parcel &data, std::vector<ShadowInfo> &shadowInfos)
{
    CALL_DEBUG_ENTER;
    int32_t shadowNum { 0 };
...
int32_t DragDataPacker::MarshallingSummary(const SummaryMap &val, Parcel &parcel)
{
    WRITEINT32(parcel, static_cast<int32_t>(val.size()), E_DEVICESTATUS_READ_PARCEL_ERROR);
    for (auto const &[k, v] : val) {
        WRITESTRING(parcel, k, E_DEVICESTATUS_READ_PARCEL_ERROR);
        WRITEINT64(parcel, v, E_DEVICESTATUS_READ_PARCEL_ERROR);
    }
    return RET_OK;
}

int32_t DragDataPacker::UnmarshallingSummary(Parcel &parcel, SummaryMap &val)
{
    int32_t size = 0;
